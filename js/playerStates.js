...
export class Punch1 extends State {
  ...
  handleInput(input) {
    if (this.player.frameX === getStrikeFrame('PUNCH1')) {
      this.player.enemies.forEach(enemy => {
        if (enemy.currentState.state !== 'BLOCK' && this.strikeRange(this.player, enemy, this.player.direction) === true) enemy.setState(states.HIT);
      });
...
export class Punch3 extends State {
  ...
  handleInput(input) {
    if (this.player.frameX === getStrikeFrame('PUNCH3')) {
      this.player.enemies.forEach(enemy => {
        if (enemy.currentState.state !== 'BLOCK' && this.strikeRange(this.player, enemy, this.player.direction) === true) enemy.setState(states.FALL);
      });
    } else if (this.player.frameX >= this.player.maxFrame) {
      this.player.setState(states.STANDING);
    }
...
function throwMove(player, enemyThrown) {
  ...
  player.enemies.forEach(enemy => {
    if (enemy.currentState.state !== 'BLOCK' && this.strikeRange(player, enemy, player.direction) === true && enemyThrown == null) enemyThrown = enemy;
  });
  if (enemyThrown !== null) {
